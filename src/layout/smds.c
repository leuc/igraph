...
static void igraph_i_smds_gower_interpolate(igraph_matrix_t *res,
    ...
    for (igraph_int_t v = 0; v < no_of_nodes; v++) {
        ...
        if (l_idx != -1) {
            MATRIX(*res, v, 0) = MATRIX(*res_sub, l_idx, 0) + mean_x;
            MATRIX(*res, v, 1) = MATRIX(*res_sub, l_idx, 1) + mean_y;
            MATRIX(*res, v, 2) = MATRIX(*res_sub, l_idx, 2) + mean_z;
        } else {
...
/* =========================================================================
 * LANDMARK INTERPOLATION SPHERICAL MDS (Gower's Interpolation)
 * ========================================================================= */

igraph_error_t igraph_layout_mds_spherical_interpolation(
    const igraph_t *graph, igraph_matrix_t *res,
    const igraph_matrix_t *dist, igraph_int_t num_iter,
    igraph_real_t lr_cap, igraph_int_t l) {
    ...
    if (l <= 0) l = 500;
    if (l > no_of_nodes) l = no_of_nodes;
    ...
    for (i = 0; i < l; i++) {
        mean_x += MATRIX(res_sub, i, 0);
        mean_y += MATRIX(res_sub, i, 1);
    ...
    for (i = 0; i < l; i++) {
        igraph_real_t nx = MATRIX(res_sub, i, 0);
        igraph_real_t ny = MATRIX(res_sub, i, 1);
    ...
    IGRAPH_PROGRESS("Spherical MDS layout (interpolation)", 80, NULL);
    ...
    igraph_vector_destroy(&q_vector); igraph_matrix_destroy(&res_sub);
